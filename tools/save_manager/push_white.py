#!/usr/bin/env python3
"""
Push Pokemon White Version save file to Google Drive
Uploads local White save and backs up existing files on Google Drive
"""
...
# Configuration
WHITE_FOLDER_ID = '1mg-XzjiEcZPw-IOGIDfTgzY-uwabf4Pz'  # Target folder for save
...
def push_white_save():
    """Main function to push White save file to Google Drive"""
    
    print("⚪ **POKEMON WHITE VERSION SAVE PUSHER**\n")
    ...
    if not os.path.exists(local_save_path):
        print(f"❌ Local White save file not found: {local_save_path}")
        print(f"💡 Run pull_white.py first to get a save file")
        return False
    ...
    existing_file = save_manager.find_file_in_folder(WHITE_FOLDER_ID, WHITE_SAVE_NAME)
    ...
    uploaded_file_id = save_manager.upload_file(
        ...
        file_name=WHITE_SAVE_NAME
    )
    
    if uploaded_file_id:
        print(f"\n🎉 **WHITE SAVE PUSHED SUCCESSFULLY!**")
        print(f"📄 Uploaded: {WHITE_SAVE_NAME}")
        print(f"📂 To folder: {WHITE_FOLDER_ID}")
        ...
        print(f"❌ Failed to upload White save file")
        return False
...
def list_drive_backups(save_manager: GoogleDriveSaveManager):
    ...
    try:
        ...
        if backup_files:
            white_backups = [f for f in backup_files if 'white' in f['name'].lower() and 'backup_' in f['name']]
            
            if white_backups:
                total_count = len(white_backups)
                recent_count = min(3, total_count)  # Show max 3 recent backups
                
                # Sort by name (which includes timestamp)
                white_backups.sort(key=lambda x: x['name'], reverse=True)
                
                print(f"\n📦 **GOOGLE DRIVE BACKUPS:** {total_count} total, showing {recent_count} most recent:")
                
                for backup in white_backups[:recent_count]:
                    print(f"  📄 {backup['name']}")
            else:
                print(f"\n📦 **GOOGLE DRIVE BACKUPS:** No White backup files found")
        else:
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print(f"\n✅ White save is now on Google Drive!")
    else:
        print(f"\n❌ Failed to push White save")
