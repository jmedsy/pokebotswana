...
local PK_KEY_NAMES = { "A", "B", "SELECT", "START", "LEFT", "RIGHT", "UP", "DOWN", "R", "L" }
local pk_key_states = {}

function pk_reset_key_states(ks)
	for _, v in ipairs(PK_KEY_NAMES) do
		ks[v] = false
	end
end

function pk_update_key_states(ks, new_ks)
	for k, v in pairs(new_ks) do
		if ks[k] ~= nil then
			ks[k] = v
		end
	end
